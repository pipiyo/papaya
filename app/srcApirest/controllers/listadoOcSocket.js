...
module.exports = (io) => {
  ...
  socket.on('allOc', (filtro,callback) => {
    ...
    let query = `select orden_de_compra.CODIGO_OC, usuario.NOMBRE_USUARIO, max(oc_recibo.fecha_recibido) as fecha_recibido, orden_de_compra.ROCHA_PROYECTO, orden_de_compra.VERSION, orden_de_compra.NOMBRE_PROVEEDOR, orden_de_compra.FECHA_REALIZACION, orden_de_compra.FECHA_ENTREGA, orden_de_compra.FECHA_CONFIRMACION, orden_de_compra.CODIGO_USUARIO, orden_de_compra.NETO, orden_de_compra.FACTURAS, orden_de_compra.FECHA_ENVIO_VALIJA, orden_de_compra.ESTADO from (orden_de_compra LEFT JOIN oc_recibo on oc_recibo.CODIGO_OC = orden_de_compra.CODIGO_OC) LEFT JOIN usuario ON usuario.CODIGO_USUARIO = orden_de_compra.CODIGO_USUARIO where ${q_estado} ${q_codigo} ${q_rocha} ${q_proveedor} ${q_fecha} group by orden_de_compra.CODIGO_OC  limit ${filtro.limit} , ${filtro.limitB} ;`
